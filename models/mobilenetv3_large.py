...
class MobileNetV3_L(Module):
    ...
    def _construct_imagenet(self):
        head_w=cfg.MBL.HEAD_W
        se_r=cfg.MBL.SE_R
        self.stem=MBSStemIN(3,16)
        # w_in, w_exp, k, stride, se_r, w_out,af
        self.s1_1=MBConv(16,16,3,1,0,16,'RE')
        self.s1_2=MBConv(16,64,3,2,0,24,'RE')

        self.s2_1=MBConv(24,72,3,1,0,24,'RE')
        self.s2_2=MBConv(24,72,5,2,se_r,40,'RE')

        self.s3_1=MBConv(40,120,5,1,se_r,40,'RE')
        self.s3_2=MBConv(40,120,3,1,se_r,40,'RE')
        self.s3_3=MBConv(40,240,3,2,se_r,80,'HS')

        self.s4_1=MBConv(80,200,3,1,0,80,'HS')
        self.s4_2=MBConv(80,184,3,1,0,80,'HS')
        self.s4_3=MBConv(80,184,3,1,0,80,'HS')
        self.s4_4=MBConv(80,480,3,1,se_r,112,'HS')
        self.s4_5=MBConv(112,672,3,1,se_r,112,'HS')
        self.s4_6=MBConv(112,672,5,2,se_r,160,'HS')

        self.s5_1=MBConv(160,960,5,1,se_r,160,'HS')
        self.s5_2=MBConv(160,960,5,1,se_r,160,'HS')

        self.head=MBSHead(160,960,head_w,cfg.MODEL.NUM_CLASSES)
    ...
    def complexity(cx):
        ...
        cx=MBConv.complexity(cx,16,16,3,1,0,16)
        cx=MBConv.complexity(cx,16,64,3,2,0,24)

        cx=MBConv.complexity(cx,24,72,3,1,0,24)
        cx=MBConv.complexity(cx,24,72,5,2,se_r,40)

        cx=MBConv.complexity(cx,40,120,5,1,se_r,40)
        cx=MBConv.complexity(cx,40,120,3,1,se_r,40)
        cx=MBConv.complexity(cx,40,240,3,2,se_r,80)

        cx=MBConv.complexity(cx,80,200,3,1,0,80)
        cx=MBConv.complexity(cx,80,184,3,1,0,80)
        cx=MBConv.complexity(cx,80,184,3,1,0,80)
        cx=MBConv.complexity(cx,80,480,3,1,se_r,112)        
        cx=MBConv.complexity(cx,112,672,3,1,se_r,112)
        cx=MBConv.complexity(cx,112,672,5,2,se_r,160)

        cx=MBConv.complexity(cx,160,960,5,1,se_r,160)
        cx=MBConv.complexity(cx,160,960,5,1,se_r,96)

        cx=MBSHead.complexity(cx,160,960,head_w,cfg.MODEL.NUM_CLASSES)
        return cx
